...
#   To report a bug or issue, use the following form:
...
# --
# This function adjusts matplotlib setteing for a uniform feel in the textbook.
...
log_V_sphere = (np.log(2) + 0.5 * dims * np.log(np.pi) - np.log(dims) - gammaln(0.5 * dims))
...
ax.semilogy(dims, np.exp(log_V_cube), '-k', label='side-2 hypercube')
ax.semilogx(dims, np.exp(log_V_sphere), '--k', label='inscribed unit hypersphere')
